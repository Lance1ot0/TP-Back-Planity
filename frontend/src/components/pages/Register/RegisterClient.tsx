...
import { registerClientApi } from "../../../api/register/registerClientApi/registerClientApi";
...
export default function RegisterClient() {
  ...
  const onHandleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const response = await registerClientApi(item);
    if (response.success) {
  ...
  return (
    <div className={style.loginWrapper}>
      <form className={style.loginForm} onSubmit={onHandleLogin}>
        <h2 className={style.loginTitle}>Register Client</h2>
      ...
      <a className={style.link} href="/client/login">
        Se connecter
      </a>
      {errorMessage && (
